...
export default async function handler(req,res){
  try{
    ...
    if(!Array.isArray(k)||!Array.isArray(k15)||!Array.isArray(k1h))return res.status(400).json({error:"Binance TR mum verisi eksik."});
    const u=unpack(k,true);
    if(u.c.length<220)return res.status(422).json({error:"Yeterli Binance TR mum verisi yok."});
    ...
    return res.status(200).json({
      ...
      v7:{version:"7.5-TR",profile,thresholds:advanced.thresholds,mtf,blockers:advanced.blockers,confirmations:advanced.confirmations,metrics:advanced.metrics},
      dataSource:"Binance TR resmi API • veri cihazdan alındı",
