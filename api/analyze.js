...
export default async function handler(req,res){
  try{
    ...
    const micro=eligible.ok&&(direction5m==="LONG"||direction5m==="SHORT")
      ? microTriggerFromKlines(marketData.k1m,direction5m,profile)
    ...
    if(eligible.ok&&micro.ok){
      const strong=(profile==="safe")||(direction5m==="LONG"?advanced.score>=80:advanced.score<=20);
      common={
        ...common,
        decision:direction5m==="LONG"?(strong?"GÜÇLÜ LONG":"LONG"):(strong?"GÜÇLÜ SHORT":"SHORT"),
        stage:direction5m==="LONG"?"AL":"SAT",
        note:`${eligible.reason} • ${micro.reason}`
      };
    }else{
