...
export default async function handler(req,res){
  try{
    ...
    if(!/^[A-Z0-9]{4,24}$/.test(symbol))return res.status(400).json({error:"Geçersiz Binance TR paritesi."});
    ...
    const [k,k15,k1h]=await Promise.all([
      trKlines(symbol,interval,320),
      trKlines(symbol,"15m",240),
      trKlines(symbol,"1h",240)
    ]);
    const u=unpack(k,true);
    if(u.c.length<220)return res.status(422).json({error:"Binance TR üzerinde yeterli mum verisi yok."});
    ...
    const h1=unpack(k1h,false);
    const prev24=h1.c.length>24?h1.c[h1.c.length-25]:null;
    const change24=prev24?((livePrice/prev24)-1)*100:0;

    res.setHeader("Cache-Control","no-store");
    return res.status(200).json({
      symbol,displaySymbol:symbol,exchange:"binancetr",marketType:"SPOT",quoteAsset:symbol.endsWith("TRY")?"TRY":"",
      interval,price:livePrice,signalClose:u.c[n],change24,
      legacy,advanced,common,levels,
      spotNote:common.decision.includes("SHORT")
        ?"SAT sinyali spot içindir: elinde coin varsa satış/azaltma düşünülebilir; coin yoksa short pozisyon açılmaz."
        :"Binance TR Spot verisi kullanılıyor.",
      v7:{version:"7.4-TR",profile,thresholds:advanced.thresholds,mtf,blockers:advanced.blockers,confirmations:advanced.confirmations,metrics:advanced.metrics},
      dataSource:"Binance TR resmi market data API",
      timestamp:new Date().toISOString()
    });
  }catch(e){return res.status(500).json({error:e?.message||"Binance TR analiz hatası."})}
}
