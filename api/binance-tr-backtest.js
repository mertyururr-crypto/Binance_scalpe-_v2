...
export default async function handler(req,res){
  try{
    ...
    const [k,k15,k1h]=await Promise.all([
      trKlines(symbol,interval,1000),
      trKlines(symbol,"15m",1000),
      trKlines(symbol,"1h",500)
    ]);
    const u=unpack(k,false),d=calcAll(u.o,u.h,u.l,u.c,u.v);
    ...
    return res.status(200).json({
      ...
      methodology:`Binance TR Spot kendi mum verisi • V7.4 ${profile} • sonraki mum açılışı • 12 mum horizon • TP 1.55 ATR / SL 1.15 ATR • aynı mumda TP+SL görülürse LOSS.`,
