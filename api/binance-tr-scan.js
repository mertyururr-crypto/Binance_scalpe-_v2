...
export default async function handler(req,res){
  try{
    ...
    for(let i=0;i<selected.length;i+=4){
      ...
      const rows=await Promise.all(chunk.map(async x=>{
        try{
          const k=await trKlines(x.symbol,"5m",240);
          const u=unpack(k,true);if(u.c.length<220)return null;
    ...
    const candidates=baseRows.filter(x=>x.rough.score>=cfg.candidateLong-4||x.rough.score<=cfg.candidateShort+4).slice(0,10);
    const final=[];
  ...
  }catch(e){return res.status(500).json({error:e?.message||"Binance TR tarama hatası."})}
}
